...
class IndexedString(object):
    ...
    def get_sentence(self, bitmask):
        '''
        bitmask: vector of 0 and 1's on what worsd to choose
        '''
        words = []
        for i in range(len(bitmask)):
            if bitmask[i] == 1:
                words.append(self.as_list[self.positions[i]])
...
class LimeTextExplainer(object):
    ...
    def explain_json_instance(self,
                         text_instance,
                         classifier_fn,
                         data,
                         labels=(1,),
        ...
        indexed_string = IndexedString(text_instance, bow=False,
                                       split_expression=self.split_expression)
        domain_mapper = TextDomainMapper(indexed_string)

        data, yss, distances = self.__data_labels_distances_from_json(
            indexed_string, classifier_fn, data,
            distance_metric=distance_metric)
    ...
    def __data_labels_distances_from_json(cls,
                                indexed_string,
                                classifier_fn,
                                data,
                                distance_metric='cosine'):
        ...
        for i in range(num_samples):
            inverse_data.append(indexed_string.get_sentence(data[i]))
